...
Adds small isotropic noise to coordinates during training before hash grid lookup.
Rationale: Acts as a regularizer, preventing high-frequency "salt-and-pepper" noise 
in the hash grid and reducing aliasing.
"""
import torch
...
class NeRFNetwork(NeRFRenderer):
    def __init__(self,
        ...
        # Jitter scale is roughly the size of the finest voxel
        self.jitter_scale = 1.0 / 2048
    ...
    def forward(self, x, d):
        # === MODIFICATION: Stochastic Jittering during training ===
        if self.training:
    ...
    def density(self, x):
        # No jittering during density estimation (used for grid update)
        encoded = self.encoder(x, bound=self.bound)
        h = encoded
        for l in range(self.num_layers):
            h = self.sigma_net[l](h)
