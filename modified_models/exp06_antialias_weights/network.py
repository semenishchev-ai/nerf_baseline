"""
EXP-06: Scale-Aware Level Weighting (Lightweight Anti-Aliasing)
Weights hash grid levels based on point distance from camera, suppressing
fine levels for distant points.
Source: Simplified version of Barron et al., "Zip-NeRF: Anti-Aliased Grid-Based
Neural Radiance Fields", ICCV 2023. No PyTorch implementation for torch-ngp exists.
"""
import torch
...
class NeRFNetwork(NeRFRenderer):
    ...
    def _apply_scale_weights(self, encoded, x):
        """Apply scale-aware weights based on distance from origin (proxy for camera distance)."""
        B = encoded.shape[0]
        L = self.num_levels
        C = self.level_dim
        ...
        weights = torch.sigmoid(
            (self.log_resolutions.unsqueeze(0) - log_scale) / tau
        )
