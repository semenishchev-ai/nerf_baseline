...
class NeRFNetwork(NeRFRenderer):
    def __init__(self,
        ...
        for l in range(num_layers):
            if l == 0:
                in_dim = self.in_dim + self.in_dim_time + self.in_dim_deform                    
            else:
                in_dim = hidden_dim
    ...
    def forward(self, x, d, t):
        ...
        enc_ori_x = self.encoder_deform(x, bound=self.bound)         
        enc_t = self.encoder_time(t)                     
        if enc_t.shape[0] == 1:
            enc_t = enc_t.repeat(x.shape[0], 1)                      

        deform = torch.cat([enc_ori_x, enc_t], dim=1)              
