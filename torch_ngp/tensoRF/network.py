...
class NeRFNetwork(NeRFRenderer):
    def __init__(self,
        ...
        if self.bg_radius > 0:
            ...
            self.bg_mat = nn.Parameter(0.1 * torch.randn((1, bg_rank, bg_resolution[0], bg_resolution[1])))               
    ...
    def init_one_svd(self, n_component, resolution, scale=0.1):
        ...
        for i in range(len(self.vec_ids)):
            ...
            mat.append(nn.Parameter(scale * torch.randn((1, n_component[i], resolution[mat_id_1], resolution[mat_id_0]))))               
            vec.append(nn.Parameter(scale * torch.randn((1, n_component[i], resolution[vec_id], 1))))                                            
    ...
    def get_sigma_feat(self, x):
        ...
        for i in range(len(self.sigma_mat)):
            mat_feat = F.grid_sample(self.sigma_mat[i], mat_coord[[i]], align_corners=True).view(-1, N)                          
            vec_feat = F.grid_sample(self.sigma_vec[i], vec_coord[[i]], align_corners=True).view(-1, N)         
    ...
    def get_color_feat(self, x):
        ...
        for i in range(len(self.color_mat)):
            mat_feat.append(F.grid_sample(self.color_mat[i], mat_coord[[i]], align_corners=True).view(-1, N))                          
            vec_feat.append(F.grid_sample(self.color_vec[i], vec_coord[[i]], align_corners=True).view(-1, N))         
    ...
    def background(self, x, d):
        ...
        h = F.grid_sample(self.bg_mat, x.view(1, N, 1, 2), align_corners=True).view(-1, N).T.contiguous()                    
